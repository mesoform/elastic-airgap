...
def check_lib_deps():
    ...
    try:
        ...
        print
        "Script requires python-requests library:"
        print
        "  https://github.com/kennethreitz/requests/releases"
        print
        "Script requires beautifulsoup4 library:"
        print
...
def parse_hashes(file_to_parse, file_to_hash):
    ...
    with open(file_to_parse, 'r') as hashfilecontents:
        for line in hashfilecontents:
            if filename in line or (lines == 1):
                if search_md5(line, True):
                    print
                    "Actual MD5:   " + calculate_md5(file_to_hash)
                    found = True
                if search_sha1(line, True):
                    print
                    "Actual SHA1:   " + calculate_sha1(file_to_hash)
                    found = True
                if search_sha256(line, True):
                    print
                    "Actual SHA256:   " + calculate_sha256(file_to_hash)
                    found = True
        if (found == False):
        ...
        if (found == False):
            print
            "Error: Valid hash not found in hashfile."
...
def search_md5(string_to_search_in, should_print):
    ...
    if md5search and should_print:
        print
        "Expected MD5: " + md5search.group(2)
...
def search_sha1(string_to_search_in, should_print):
    ...
    if sha1search and should_print:
        print
        "Expected SHA1: " + sha1search.group(2)
    return sha1search
...
def search_sha256(string_to_search_in, should_print):
    ...
    if sha256search and should_print:
        print
        "Expected SHA256: " + sha256search.group(2)
    return sha256search
...
def scrape_elastic(page_to_scrape):
    ...
    if [[page.status_code == requests.codes.ok]]:
        ...
        anchors = soup.find_all('a', href=re.compile("(?<!.aarch64.)rpm"))
...
def main():
    ...
    if not os.path.exists(args.directory):
        print
        "ERROR: Path \"" + os.path.abspath(args.directory) + "\" doesn't exist"
        sys.exit(1)
